...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化内测声明，以及按条件显示的 DeepSeek 官方凭据步骤。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把 `llm.providers` 与 `llm.authStatus`（可配置路由、存活状态、支持的登录方法和非机密认证状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）及 `credentials.describe`（不含值的 configured/source/writable 徽标）汇聚为一个共享快照；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。

支持 OAuth 的行通过通用 `llm` API 启动、恢复、取消和登出提供方拥有的认证。运行中的操作显示提供方通知，包括 GitHub 验证 URL 与设备码，并且只在状态为 `running` 时轮询；页面刷新通过稳定 id 恢复同一个操作。prompt 回答可以穿过协议，但 access token 与 refresh token 绝不会。成功后会重新获取非机密状态，只有 OAuth 已配置时，该提供方才计入引导就绪判断。
