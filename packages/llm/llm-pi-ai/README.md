...
OAuth-capable catalog routes expose `oauth` through the configurable-provider directory. `ctx.llm.providerLogin()` delegates the provider interaction to pi-ai and stores its canonical OAuth credential as a versioned JSON secret through `ctx.credentials`; access and refresh tokens never enter settings, session logs, or provider-directory responses. Every immutable `Models` snapshot receives the same Harness-backed credential store. pi-ai checks expiry and runs refresh inside the store's per-provider `modify`, so concurrent requests in one process perform one refresh and persist token rotation before dispatch. `ctx.llm.providerLogout()` deletes that provider-owned secret. A missing credentials service permits reads but rejects login and refresh writes; a read-only shadowing credential source is reported by the credential provider.
...
API keys never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override, preserving the harness's fail-loud reference semantics. OAuth credentials are the exception: every collection receives the shared Harness-backed store so pi-ai can resolve and refresh them under its locked read-modify-write protocol. A route naming neither an explicit key reference nor a stored OAuth credential leaves authentication to the provider's ambient rules.
...
- **OAuth refresh serialization is process-local unless the credential provider supplies a stronger lock** — the Harness store serializes `modify` per provider inside one process. Two Harness processes sharing one credential home can both refresh the same expired credential when the backing provider does not implement cross-process exclusion; each completed write remains a valid canonical credential and later reads observe the last writer.
