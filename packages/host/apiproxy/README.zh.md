...
对于 web 提供方配置，同一份显式 allowlist 还包括 `web`（共享的搜索／抓取选择）和由树外提供方组合包拥有的 `web-search-microsoft-webiq`。二者仍是该网关策略中的显式条目；仅注册任一 namespace 都不会自动暴露它——这也正是可安装插件没有这里的条目就无法提供自己的配置分节的原因。
