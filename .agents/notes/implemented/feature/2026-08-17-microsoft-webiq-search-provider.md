...
The package's browser half registers a Web IQ card only when `settings.plugin.item` exists. The card shows whether `WEBIQ_API_KEY` or the configured credential reference resolves, accepts a replacement key through the credentials RPC, exposes the provider's non-secret settings, and offers a control that stages `web.searchProvider = "microsoft-webiq"`. Saving writes secrets through the credentials domain and ordinary values through their owning settings namespaces. Secret literals remain write-only and are blank after every load.
