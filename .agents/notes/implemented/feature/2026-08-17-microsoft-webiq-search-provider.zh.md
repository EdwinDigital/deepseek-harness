...
该包的浏览器半包只在 `settings.plugin.item` 存在时注册 Web IQ 卡片。卡片显示 `WEBIQ_API_KEY` 或所配置凭据引用能否解析，通过凭据 RPC 接受替换密钥，开放提供方的非 secret 设置，并提供一个用于暂存 `web.searchProvider = "microsoft-webiq"` 的控件。保存时，secret 通过凭据领域写入，普通值通过各自所属的设置命名空间写入。secret 字面量保持只写，每次加载后均为空白。
